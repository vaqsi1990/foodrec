...
app.use(cookieParser())
  ...
  app.post('/food',  (req, res) => {
    ...
    const {
      title,ingredients,addedPhotos,instructions,time,category,nutritions, difficulty
      
    } = req.body;
    jwt.verify(token, "ILOVEANNA", {}, async (err, userData) => {
  const FoodDoc =  await  Food.create({
        owner: userData.id,
        title,ingredients,instructions,time,photos:addedPhotos,category,nutritions, difficulty
      })
