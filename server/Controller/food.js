...
exports.editFood = async(req, res, next) => {
    ...
    const { token } = req.cookies;
    const {
     id, title,ingredients,addedPhotos,instructions,time,category, difficulty, nutritions
    ...
    jwt.verify(token, "ILOVEANNA", {}, async (err, userData) => {
       ...
       if(userData.id === placeDoc.owner.toString()) {
        placeDoc.set({
            ...
            title,ingredients,instructions,time,photos:addedPhotos,category,difficulty,  nutritions
          })
       await placeDoc.save()
        res.json('ok')
       }
